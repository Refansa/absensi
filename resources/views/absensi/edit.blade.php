...
@section('body')
  <main class="mx-4 my-8">
    ...
    <section>
      <form class="py-4 px-64 border-2 border-black flex flex-col gap-5" action="{{ route('absensi.update', $absensi) }}"
        method="POST">
        @csrf
        @method('PUT')
        ...
        <select class="border-2 border-black outline-none p-1 bg-white h-9" name="id_siswa" id="siswa" required>
          @foreach ($siswa as $s)
            <option {{ $absensi->id_siswa == $s->id_siswa ? 'selected' : '' }} value="{{ $s->id_siswa }}">
              {{ $s->nama }}</option>
          @endforeach
        </select>
        ...
        <select class="border-2 border-black outline-none p-1 bg-white h-9" name="keterangan" id="keterangan" required>
          @foreach (['Masuk', 'Izin', 'Sakit', 'Alfa'] as $value)
            <option {{ $absensi->keterangan == $value ? 'selected' : '' }} value="{{ $value }}">
              {{ $value }}</option>
          @endforeach
        </select>

...
@section('body')
  <main class="mx-4 my-8">
    ...
    <section>
      <form class="py-4 px-64 border-2 border-black flex flex-col gap-5" action="{{ route('siswa.store') }}"
        method="POST">
        @csrf
        <h1 class="text-center font-bold text-xl mb-4">Tambah</h1>
        <label class="text-sm font-medium w-64" for="nama">Nama</label>
        <input class="border-2 border-black outline-none p-1" type="text" name="nama" id="nama" maxlength="255"
          required>
        <label class="text-sm font-medium w-64" for="email">Email</label>
        <input class="border-2 border-black outline-none p-1" type="email" name="email" id="email" maxlength="255"
          required>
        <label class="text-sm font-medium w-64" for="alamat">Alamat</label>
        <input class="border-2 border-black outline-none p-1" type="text" name="alamat" id="alamat" maxlength="255"
          required>
        <label class="text-sm font-medium w-64" for="notelp">No. Telp</label>
        <input class="border-2 border-black outline-none p-1" type="text" name="notelp" id="notelp" maxlength="255"
          required>
